...
public final class ObjectLogUtils {
    ...
    private final int FILE = 0x10;
    private final int JSON = 0x20;
    private final int XML  = 0x30;
...
//        throw new UnsupportedOperationException("u can't instantiate me...");
    ...
    public static class Builder {
        ...
        private boolean sLog2ConsoleSwitch = true; // logcat是否打印，默认打印
        ...
        private int     sConsoleFilter     = V;    // log控制台过滤器
        private int     sFileFilter        = V;    // log文件过滤器
        private String          defaultDir;// log默认存储目录
        private String          dir;       // log存储目录

        public Builder() {
            if (this.defaultDir != null) return;
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && CoreUtil.getContext().getExternalCacheDir() != null)
                this.defaultDir = CoreUtil.getContext().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                this.defaultDir = CoreUtil.getContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
    ...
    public void v(Object contents) {
        log(V, builder.sGlobalTag, contents);
    }
    ...
    public void d(Object contents) {
        log(D, builder.sGlobalTag, contents);
    }
    ...
    public void i(Object contents) {
        log(I, builder.sGlobalTag, contents);
    }
    ...
    public void w(Object contents) {
        log(W, builder.sGlobalTag, contents);
    }
    ...
    public void e(Object contents) {
        log(E, builder.sGlobalTag, contents);
    }
    ...
    public void a(Object contents) {
        log(A, builder.sGlobalTag, contents);
    }
    ...
    public void file(Object contents) {
        log(FILE | D, builder.sGlobalTag, contents);
    }

    public void file(@TYPE int type, Object contents) {
        log(FILE | type, builder.sGlobalTag, contents);
    }
    ...
    public void file(@TYPE int type, String tag, Object contents) {
        log(FILE | type, tag, contents);
    }

    public void json(String contents) {
        log(JSON | D, builder.sGlobalTag, contents);
    }

    public void json(@TYPE int type, String contents) {
        log(JSON | type, builder.sGlobalTag, contents);
    }
    ...
    public void json(@TYPE int type, String tag, String contents) {
        log(JSON | type, tag, contents);
    }

    public void xml(String contents) {
        log(XML | D, builder.sGlobalTag, contents);
    }

    public void xml(@TYPE int type, String contents) {
        log(XML | type, builder.sGlobalTag, contents);
    }
    ...
    public void xml(@TYPE int type, String tag, String contents) {
        log(XML | type, tag, contents);
    }

    private void log(final int type, String tag, final Object... contents) {
        if (!builder.sLogSwitch || (!builder.sLog2ConsoleSwitch && !builder.sLog2FileSwitch)) return;
        int type_low = type & 0x0f, type_high = type & 0xf0;
        if (type_low < builder.sConsoleFilter && type_low < builder.sFileFilter) return;
        final String[] tagAndHead = processTagAndHead(tag);
        String body = processBody(type_high, contents);
        if (builder.sLog2ConsoleSwitch && type_low >= builder.sConsoleFilter) {
            print2Console(type_low, tagAndHead[0], tagAndHead[1] + body);
        }
        if (builder.sLog2FileSwitch || type_high == FILE) {
            if (type_low >= builder.sFileFilter) print2File(type_low, tagAndHead[0], tagAndHead[2] + body);
        }
    }

    private String[] processTagAndHead(String tag) {
        if (!builder.sTagIsSpace && !builder.sLogHeadSwitch) {
            tag = builder.sGlobalTag;
        } else {
            ...
            if (builder.sTagIsSpace) {
                tag = isSpace(tag) ? className : tag;
            }
            if (builder.sLogHeadSwitch) {
    ...
    private void print2Console(final int type, String tag, String msg) {
        if (builder.sLogBorderSwitch) {
        ...
        if (countOfSub > 0) {
            ...
            for (int i = 1; i < countOfSub; i++) {
                ...
                print(type, tag, builder.sLogBorderSwitch ? LEFT_BORDER + sub : sub);
                index += MAX_LEN;
            }
        ...
        if (builder.sLogBorderSwitch) print(type, tag, BOTTOM_BORDER);
    }
    ...
    private String addLeftBorder(String msg) {
        if (!builder.sLogBorderSwitch) return msg;
        StringBuilder sb = new StringBuilder();
    ...
    private void print2File(final int type, final String tag, final String msg) {
        Date now = new Date(System.currentTimeMillis());
        String format = FORMAT.format(now);
        String date = format.substring(0, 5);
        String time = format.substring(6);
        final String fullPath = (builder.dir == null ? builder.defaultDir : builder.dir) + date + ".txt";
